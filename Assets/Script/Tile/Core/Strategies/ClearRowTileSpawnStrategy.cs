...
public class ClearRowTileSpawnStrategy : BaseTileSpawnStrategy
{
    public override int Priority => 1;
    public override TileEffect TileEffect => TileEffect.Special_Clear_Row;
    ...
    public override bool ShouldSpawnSpecialTile(List<List<Tile>> board,int matchSize, Dictionary<Vector2Int,Tile> matchedTiles, SwapDirection swapDirection)
    {
        ...
        bool isHorizontalSwap = swapDirection == SwapDirection.Horizontal;
        foreach (var tilePosition in matchedTiles.Keys)
        {
            if (isHorizontalSwap && CheckConditional(board, tilePosition, matchSize+1))
            {
